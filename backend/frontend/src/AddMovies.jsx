...
import { createMovies } from "./utils/axiosInstance";
import { CgSpinner } from "react-icons/cg";
import { useMoviesMutation } from "./utils/hooks/useUserMutation";
import { useUpdateMovie } from "./utils/hooks/useUpdateMovies";
import Navbar from './Navbar'
import Footer from './Footer'

 

const AddMovies = ({ data, route }) => {
  const { mutateAsync, isLoading: load } = useUpdateMovie();
  

  const { mutateAsync: mutate, isLoading } = useMoviesMutation();
  ...
  const onSubmit = async (e) => {
    ...
    if (route) {
      const datas = await mutateAsync({ id: data?.data?.data?._id, newForm });
      

      console.log("addmovieupdayedata", datas);
    } else {
      const data = await mutate(newForm);
      setformData('')
    }
  };
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-4 items-center my-2 ">
              ...
              <Input
                ...
                name="isMovies"
                defaultValue={"series"}
                checked={formData?.isMovies === "series"}
                id="noid"
                onChange={inputHandler}
